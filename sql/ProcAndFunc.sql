...
create or replace function get_product_comments(productId integer)
	returns table(user_name text,user_lastname text,user_email text,comment_date date,comment_text text)
as $body$
	begin
		return query 
			...
			from product_comments  as com
				join users as usr on com.user_id=usr.user_id and com.product_id=productId;
			
	end;
$body$ language plpgsql;
----------------------------------------Создать комментарий-------------------------------------------------
create sequence comment_id start 1 no cycle;
create or replace procedure add_comment(producId bigint, userId bigint,commentText varchar(1023))
as $body$
	begin
